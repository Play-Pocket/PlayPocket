...
importFile.addEventListener('change', async (e) => {
  ...
  try {
    if (f.size > MAX_IMPORTED_JSON_BYTES) {
    ...
    for (const it of obj.items.slice(0, MAX_IMPORTED_ITEMS)) {
      ...
      const thumbnail = typeof it.thumbnail === 'string' && it.thumbnail.length < 2_000_000 ? it.thumbnail : null;

      if (typeof it.blobBase64 === 'string' && it.blobBase64.length > 0) {
        const blob = base64ToBlob(it.blobBase64, 'video/mp4');
        const meta = { id, name: metaName, duration, blob, thumbnail, size };
        await idbPut(STORE_VIDEOS, meta);
        videoListCache[id] = meta;
        items.push(id);
      } else {
        const meta = { id, name: metaName, duration, blob: null, thumbnail, size };
        await idbPut(STORE_VIDEOS, meta);
        videoListCache[id] = meta;
        items.push(id);
      }
...
async function init() {
  ...
  vids.forEach(v => {
    if (v && v.id) videoListCache[v.id] = v;
  });
